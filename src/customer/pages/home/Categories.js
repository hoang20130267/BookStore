export const Categories = () => {
    return (
        <div className="wp-block-bwgb-product-categories__inner container">
            <header
                className="bwgb-products-category__block-header d-md-flex justify-content-between align-items-center mb-5">
                <h2 className="bwgb-product-category__block-title font-size-7 mb-3 mb-md-0 mb-3 mb-md-0">Danh mục nổi
                    bật</h2><a href="src/customer/pages/home/Home#"
                               className="h-primary d-block bwgb-products-category__block-action-text bwgb-product-category__block-title"><span>Tất cả danh mục</span><i
                className="fa-solid fa-caret-right ml-1"></i></a></header>
            <div className="wp-block-bwgb-product-categories__content">
                <div>
                    <div
                        className="row no-gutters row-cols-1 row-cols-md-1 row-cols-lg-3 row-cols-xl-3 row-cols-wd-3 border-top border-left">
                        <div className="col">
                            <div className="position-relative h-100">
                                <div className="border-bottom border-right p-4 p-lg-7 h-100">
                                    <h6 className="font-size-3 mb-3 pb-1 bwgb-products-category__category-title">
                                        Food &amp; Drink </h6>
                                    <ul className="list-unstyled m-0 p-0 bwgb-products-category__category-list">
                                        <li className="cat-item cat-item-153"><a
                                            href="https://bookworm.madrasthemes.com/product-category/b/beverages/">Beverages</a>
                                        </li>
                                        <li className="cat-item cat-item-154"><a
                                            href="https://bookworm.madrasthemes.com/product-category/b/celebrity-chefs/">Celebrity
                                            Chefs</a>
                                        </li>
                                        <li className="cat-item cat-item-155"><a
                                            href="https://bookworm.madrasthemes.com/product-category/b/cigars-smoking/">Cigars &amp; Smoking</a>
                                        </li>
                                        <li className="cat-item cat-item-156"><a
                                            href="https://bookworm.madrasthemes.com/product-category/b/cookery-by-ingredient/">Cookery
                                            By Ingredient</a>
                                        </li>
                                    </ul>
                                    <div
                                        className="d-flex d-md-block justify-content-end position-md-absolute bottom-0 right-md-30 product-category__icon">
                                        <i className="fa-solid fa-utensils text-tangerine__1 font-size-17"></i>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="col">
                            <div className="position-relative h-100">
                                <div className="border-bottom border-right p-4 p-lg-7 h-100">
                                    <h6 className="font-size-3 mb-3 pb-1 bwgb-products-category__category-title">
                                        Romance </h6>
                                    <ul className="list-unstyled m-0 p-0 bwgb-products-category__category-list">
                                        <li className="cat-item cat-item-157"><a
                                            href="https://bookworm.madrasthemes.com/product-category/c/cookery-dishes-courses/">Cookery
                                            Dishes &amp; Courses</a>
                                        </li>
                                        <li className="cat-item cat-item-158"><a
                                            href="https://bookworm.madrasthemes.com/product-category/c/historical-romance/">Historical
                                            Romance</a>
                                        </li>
                                        <li className="cat-item cat-item-159"><a
                                            href="https://bookworm.madrasthemes.com/product-category/c/home-nursing-caring/">Home
                                            Nursing &amp; Caring</a>
                                        </li>
                                        <li className="cat-item cat-item-160"><a
                                            href="https://bookworm.madrasthemes.com/product-category/c/popular-medicine-health/">Popular
                                            Medicine &amp; Health</a>
                                        </li>
                                        <li className="cat-item cat-item-161"><a
                                            href="https://bookworm.madrasthemes.com/product-category/c/safety-in-the-home/">Safety
                                            In The Home</a>
                                        </li>
                                    </ul>
                                    <div
                                        className="d-flex d-md-block justify-content-end position-md-absolute bottom-0 right-md-30 product-category__icon">
                                        <i className="fa-regular fa-heart bwgb-products-category__icon-2 text-chili__1 font-size-17"></i>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="col">
                            <div className="position-relative h-100">
                                <div className="border-bottom border-right p-4 p-lg-7 h-100">
                                    <h6 className="font-size-3 mb-3 pb-1 bwgb-products-category__category-title">
                                        Health </h6>
                                    <ul className="list-unstyled m-0 p-0 bwgb-products-category__category-list">
                                        <li className="cat-item cat-item-162"><a
                                            href="https://bookworm.madrasthemes.com/product-category/d/advice-on-parenting/">Advice
                                            On Parenting</a>
                                        </li>
                                        <li className="cat-item cat-item-163"><a
                                            href="https://bookworm.madrasthemes.com/product-category/d/coping-with-personal-problems/">Coping
                                            With Personal Problems</a>
                                        </li>
                                        <li className="cat-item cat-item-164"><a
                                            href="https://bookworm.madrasthemes.com/product-category/d/family-relationships/">Family &amp; Relationships</a>
                                        </li>
                                        <li className="cat-item cat-item-165"><a
                                            href="https://bookworm.madrasthemes.com/product-category/d/fitness-diet/">Fitness &amp; Diet</a>
                                        </li>
                                        <li className="cat-item cat-item-166"><a
                                            href="https://bookworm.madrasthemes.com/product-category/d/giving-up-smoking/">Giving
                                            Up Smoking</a>
                                        </li>
                                    </ul>
                                    <div
                                        className="d-flex d-md-block justify-content-end position-md-absolute bottom-0 right-md-30 product-category__icon">
                                        <i className="fa-solid fa-stethoscope bwgb-products-category__icon-3 text-carolina__1 font-size-17"></i>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}